...
#include <limits>      // for std::numeric_limitsdouble::infinity()
...
// // [[Rcpp::export()]]
void getX_dynIRT_P3(arma::mat &Ex,              // NxT: current ideal points for each legislator and time period.
                 arma::mat &Vx,                 // NxT: current ideal point variances for each legislator and time period.
                 const arma::mat &Ebb,          // Jx1: E[β^2] stacked by time
                 const arma::mat &omega2,       // Nx1: process noise per i (legislator innovation parameter)
                 const arma::mat &Eb,           // Jx1: E[β] stacked by time (item parameters stacked by time)
                 const arma::mat &Eystar,       // NxJ: E[y*]
                 const arma::mat &Eba,          // Jx1: E[β α] stacked by time
                 const arma::mat &startlegis,   // Nx1: contiguous legislator first time period
                 const arma::mat &endlegis,     // Nx1: contiguous legislator last time period
                 const arma::mat &prevlegis,    // Nx1: legislator ID previous iteration of legislator (1, ..., N)
                 const arma::mat &xmu0,         // Nx1: prior mean at entry
                 const arma::mat &xsigma0,      // Nx1: prior var at entry
                 const arma::mat &xsign,        // Nx1: legislator sign constraints (left-wing -> non-positive, right-wing -> non-negative, unconstrained)
                 const int T,                   //      total number of time periods
                 const int N,                   //      total number of contiguous legislators    
                 const arma::mat &end_session,  // Tx1: end indices in {items j}
                 const arma::mat &Ep            // NxT: E[p_it]
) {
  ...
  for (int ii = 0; ii < N; ++ii) {
    ...
    // Require values in {-1,0,1} (integer-like)
    int s = (int)std::llround(v);
    if (!std::isfinite(v) || std::fabs(v - (double)s) > 1e-8 || (s != -1 && s != 0 && s != 1)) {
      Rcpp::stop("getX_dynIRT_P3: xsign[%d]=%f is invalid. Must be -1, 0, or 1.", ii+1, v);
    }
    
    double L = X_MIN, U = X_MAX;
    if (s == -1) U = std::min(U, 0.0);   // constrain to (-inf, 0]
    if (s ==  1) L = std::max(L, 0.0);   // constrain to [0, inf)
  ...
  // ----- Build effective entry prior mean for each contiguous legislator -----
  // If prevlegis(i) == 0: use xmu0(i).
  // If prevlegis(i)  > 0: use Ex(prev_idx, endlegis(prev_idx)) from the *current* Ex passed in
  //                       (i.e., last EM iteration’s values, since we haven't overwritten Ex yet).
  arma::colvec xmu0_eff(N, arma::fill::zeros);
  
  for (int ii = 0; ii < N; ++ii) {
    double mu = xmu0(ii, 0);
   
    int prev = static_cast<int>(prevlegis(ii, 0)); // 0 or 1..N (R-style)
    if (prev > 0) {
      int p = prev - 1; // convert 1..N to 0..N-1 for C++
      if (p >= 0 && p < N) {
        int t_end_prev = static_cast<int>(endlegis(p, 0));
        if (t_end_prev >= 0 && t_end_prev < T) {
          double cand = Ex(p, t_end_prev);
  ...
  #pragma omp parallel for private(t,Eystar_t,Eb_t,yDD)
	for(i=0; i < N; i++){
	  ...
	  const double mu_entry = xmu0_eff(i); // Effective entry prior mean for this contiguous legislator
	  ...
	  yDD = ( EyEb - Eba_sum(t,0) ) / denom;
		...
		if (betaDD(t,0) < BETA_EPS) {
		  ...
		  c_mean(i,t)= mu_entry;             // effective prior mean at entry
		  //c_mean(i,t)= xmu0(i,0);          // prior mean at entry
		  
		  auto mv = trunc_box(c_mean(i,t), C_var(i,t), Lb(i), Ub(i));
		  c_mean(i,t) = mv.first;
		  C_var(i,t)  = mv.second;
		} else {
		  // Measurement update
		  ...
		  c_mean(i,t)= mu_entry + Kt(i,t)*(yDD - betaDD(t,0)*mu_entry);          // c_t
		...
		if(startlegis(i,0) != endlegis(i,0)){
		  ...
		  {
		    auto mv = trunc_box(Ex(i,t1), Vx(i,t1), Lb(i), Ub(i));
		    Ex(i,t1) = mv.first;
		    Vx(i,t1) = mv.second;
		  }
		  ...
		  for(t = t1-1; t >= t0; --t){
		    ...
		    auto mv = trunc_box(Ex(i,t), Vx(i,t), Lb(i), Ub(i));
		    Ex(i,t) = mv.first;
		    Vx(i,t) = mv.second;
		  }
		} // multi-period
	...
	// ===== Overlap-based affine anchoring across adjacent periods =====
	// Align period t to period (t-1) using overlap MPs:
	//   x_{t-1} ≈ a_t + b_t * x_t
	// Then apply: x_t <- a_t + b_t * x_t,  V_t <- b_t^2 * V_t
  {
    const int    MIN_OVERLAP = 90;   // <--- this is your "min_overlap"
    const double RIDGE_W     = 1e-6;  // weight ridge to avoid division by ~0
    const double VARX_FLOOR  = 1e-10; // avoid exploding b when var(x_t) ~ 0
    const double B_MIN       = 0.70;  // clamp rescale (tighten if you want)
    const double B_MAX       = 1.40;
    
    for (int tt = 1; tt < T; ++tt) {
      
      int n_overlap = 0;
      double sw  = 0.0;  // sum weights
      double sx  = 0.0;  // sum w * x_cur
      double sy  = 0.0;  // sum w * x_prev
      double sxx = 0.0;  // sum w * x_cur^2
      double sxy = 0.0;  // sum w * x_cur * x_prev
      
      // 1) compute (a_t, b_t) from overlap MPs
      for (int ii = 0; ii < N; ++ii) {
        
        const int tprev = tt - 1;
        
        const bool in_prev = (tprev >= (int)startlegis(ii,0) && tprev <= (int)endlegis(ii,0));
        const bool in_cur  = (tt    >= (int)startlegis(ii,0) && tt    <= (int)endlegis(ii,0));
        if (!in_prev || !in_cur) continue;
        
        const double x_prev = Ex(ii, tprev);
        const double x_cur  = Ex(ii, tt);
        const double v_prev = Vx(ii, tprev);
        const double v_cur  = Vx(ii, tt);
        
        if (!std::isfinite(x_prev) || !std::isfinite(x_cur)) continue;
        if (!(v_prev >= 0.0) || !(v_cur >= 0.0)) continue;
        
        const double w = 1.0 / (v_prev + v_cur + RIDGE_W);
      ...
      // Not enough overlap -> skip anchoring for this transition
      if (n_overlap < MIN_OVERLAP || !(sw > 0.0)) continue;
      
      const double mx   = sx / sw;
      const double my   = sy / sw;
      const double varx = sxx / sw - mx * mx;
      const double cov  = sxy / sw - mx * my;
      
      // If overlap x_t has essentially no spread, skip (b would explode)
      if (!(varx > VARX_FLOOR)) continue;
      
      double b = cov / varx;
      // Enforce no sign flip (optional but recommended if you use xsign constraints)
      if (!(b > 0.0)) continue;
      ...
      // 2) apply transform to everyone serving in period tt
      for (int ii = 0; ii < N; ++ii) {
        if (!(tt >= (int)startlegis(ii,0) && tt <= (int)endlegis(ii,0))) continue;
        
        if (!std::isfinite(Ex(ii,tt))) continue;
        
        Ex(ii,tt) = a + b * Ex(ii,tt);
        Vx(ii,tt) = std::max(b*b * Vx(ii,tt), EPSV);
        
        // Re-apply your per-legislator truncation (xsign) after the affine transform
        auto mv = trunc_box(Ex(ii,tt), Vx(ii,tt), Lb(ii), Ub(ii));
        Ex(ii,tt) = mv.first;
        Vx(ii,tt) = mv.second;
      }
    }
