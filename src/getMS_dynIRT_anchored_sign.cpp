...
// ---- NEW: smooth inequality barrier for beta sign constraints ----
// beta_sign =  1 => enforce m >= s  (penalize s - m > 0)
// beta_sign = -1 => enforce m <= s  (penalize m - s > 0)
// beta_sign =  0 => no penalty
static inline void add_beta_sign_barrier(
    int beta_sign_val,
    double m,
    double s,
    double lambda_sign,
  ...
  if (beta_sign_val == 0) return;
  
  // z > 0 means constraint violation
  const double z = (beta_sign_val == 1) ? (s - m) : (m - s);
  
  // penalty = lambda * softplus(z)
  const double sp  = softplus(z);
  ...
  // if z = s - m: (-1, +1)
  // if z = m - s: (+1, -1)
  const double dz_dm = (beta_sign_val == 1) ? -1.0 :  1.0;
  const double dz_ds = (beta_sign_val == 1) ?  1.0 : -1.0;
  
  // grad += lambda * sigmoid(z) * dz/dtheta
  grad(0) += lambda_sign * sig * dz_dm;
  grad(1) += lambda_sign * sig * dz_ds;
  
  // Hessian += lambda * curv * (dz/dtheta)(dz/dtheta)'
  H(0,0) += lambda_sign * curv * dz_dm * dz_dm;
  H(1,1) += lambda_sign * curv * dz_ds * dz_ds;
  H(0,1) += lambda_sign * curv * dz_dm * dz_ds;
  H(1,0) += lambda_sign * curv * dz_ds * dz_dm;
}
...
void getMS_dynIRT_anchored_sign(
  ...
  // strength of beta-sign barrier (increase if you want “harder” enforcement)
  const double LAMBDA_SIGN = 25.0;
  ...
  for (uword j = 0; j < nJ; ++j) {
    ...
    for (uword it = 0; it < newton_maxit; ++it) {
      ...
      arma::mat H =
        ...
        // Add beta-sign barrier to obj/grad/H
        add_beta_sign_barrier(bs, m, s, LAMBDA_SIGN, obj, grad, H);
    ...
    // Final Hessian at (m,s) for posterior covariance
    {
      ...
      arma::mat H = S0*( gA*gA.t() + A*H_A ) + Sx*( B*H_A + gA*gB.t() + gB*gA.t() ) + Sx2*( gB*gB.t() ) - Sy*H_A + Lambda;
      ...
      // NEW: add sign barrier curvature to final Hessian (for Sigma)
      if (bs != 0) {
        const double z = (bs == 1) ? (s - m) : (m - s);
        const double sig = sigmoid(z);
        const double curv = sig * (1.0 - sig);
        
        // Hessian += lambda * curv * [[1,-1],[-1,1]]
        H(0,0) += LAMBDA_SIGN * curv;
        H(1,1) += LAMBDA_SIGN * curv;
        H(0,1) += -LAMBDA_SIGN * curv;
        H(1,0) += -LAMBDA_SIGN * curv;
      }
      ...
      // truncated write-back to box
      auto mv_m = trunc_box_scalar(m, Sigma(0,0), MS_MIN, MS_MAX);
      auto mv_s = trunc_box_scalar(s, Sigma(1,1), MS_MIN, MS_MAX);
      
      double m_w = mv_m.first;
      double s_w = mv_s.first;
      
      // NEW: ensure sign constraint still holds after truncation.
      // If violated, set m=s (=> beta=0) to satisfy either constraint.
      //if (bs == 1 && m_w < s_w) {
      //  const double c = 0.5*(m_w + s_w);
      //  m_w = c; s_w = c;
      //} else if (bs == -1 && m_w > s_w) {
      //  const double c = 0.5*(m_w + s_w);
      //  m_w = c; s_w = c;
      //}
